...
#启动命令：CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node=2 apex_pre_train.py
...
def pre_trained(config):
    ...
    for epoch in range(config.num_train_epochs):
        ...
        for batch, (input_ids, token_type_ids, attention_mask, label) in enumerate(tqdm(train_load)):
            ...
            with ac():
                loss = model(input_ids=input_ids, attention_mask=attention_mask,token_type_ids=token_type_ids, labels=label)[0]
